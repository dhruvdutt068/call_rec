...
/**
 * Authoritative Android Telecom InCallService implementation.
 *
 * Registered in AndroidManifest.xml:
 * - android:permission="android.permission.BIND_INCALL_SERVICE"
 * - android.telecom.IN_CALL_SERVICE_UI = true
 * - android.telecom.IN_CALL_SERVICE_RINGING = true
 */
@AndroidEntryPoint
class CallService : InCallService(), TelecomCallController {
    ...
    @Inject
    lateinit var callAudioController: CallAudioController
    ...
    override fun onCreate() {
        ...
        DeveloperLogger.info("CALL_SERVICE_LIFECYCLE", "CallService created and attached to Telecom.")
        callSessionManager.registerTelecomController(this)
        callAudioController.attachTelecomController(this)
    }

    override fun onDestroy() {
        super.onDestroy()
        Log.i(TAG, "CallService destroyed")
        DeveloperLogger.info("CALL_SERVICE_LIFECYCLE", "CallService destroyed and detached.")
        callAudioController.detachTelecomController(this)
    ...
    override fun onCallAdded(call: Call) {
        ...
        // Register Call.Callback to observe reactive call lifecycle
        val callback = object : Call.Callback() {
            override fun onStateChanged(c: Call, state: Int) {
                val newState = mapCallState(state)
                val caps = mapCapabilities(c.details?.callCapabilities ?: 0)
                DeveloperLogger.info("CALL_CALLBACK_STATE", "Call $callId state changed: $newState")
            ...
            override fun onDetailsChanged(c: Call, d: Call.Details) {
                ...
                DeveloperLogger.info("CALL_CALLBACK_DETAILS", "Call $callId details updated: capabilities=$caps")
                callSessionManager.onTelecomCallStateChanged(callId, newState, caps)
            }
        }
    ...
    override fun onCallAudioStateChanged(audioState: CallAudioState) {
        ...
        callAudioController.onTelecomAudioStateChanged(audioState)
